 ...
 * 同じフレーム内でも別々のタイミングで行われる。Fabric API の
 * {@link ScreenEvents#BEFORE_INIT} で各 Screen の生成を捕捉し、その Screen 用に
 * {@code beforeRender} / {@code afterRender} コールバックを登録することで、
 * "今まさに Screen#render() の中にいるか" を検出する。これにより、インベントリの
 * プレイヤードール表示（Screen#render() の内部で描画される）だけを正確に
 * 狙い撃ちでき、ワールド内の無関係な描画には影響しない。
 *
 * <h3>登録方法</h3>
 * {@code TacZMeshLoaderMod.onInitializeClient()} から {@link #register()} を呼ぶこと。
 */
public final class ScreenRenderTracker {
    ...
    /**
     * Fabric API の ScreenEvents に登録する。
     * {@code TacZMeshLoaderMod.onInitializeClient()} の中で一度だけ呼ぶこと。
     */
