    ...
    public void render(PoseStack poseStack, ItemStack stack, ItemDisplayContext transformType,
        ...
        if (this.cachedTexture == null) {
            if (this.overrideTexture != null) {
                this.cachedTexture = this.overrideTexture;
            } else {
                com.tacz.guns.api.TimelessAPI.getGunDisplay(stack).ifPresent(display -> this.cachedTexture = display.getModelTexture());
            }
        }
        if (this.cachedTexture == null) {
            super.render(poseStack, stack, transformType, renderType, light, overlay);
        ...
        //
        // 長期にわたる調査の結果、AR のレイヤー機構（setRenderLayer /
        // setRenderBeforeFunction による遅延実行）を介して poly_mesh の
        // 描画とキューブボディの AR 加速を部分的に協調させようとする試みは、
        // TacZ 本体側の内部処理（スコープのレンズ内容・ステンシル書き込みの
        // 遅延実行）との順序整合性を静的な解析だけで保証することができず、
        // スコープのレンズが真っ黒になる不具合・poly_mesh のアニメーションが
        // フリーズする不具合・Iris (Fabric) 環境での FPS 異常など、
        // 繰り返し深刻な不具合を引き起こすことが判明した。
        //
        // そのため方針を転換し、AR が有効な場合は、このメッシュ銃の描画全体
        ...
        // これまで通り正常に AR 加速される。）
        // ARCompat.disableAcceleration()/resetAcceleration() は内部で LOADED
        // (mod導入有無、クラスロード時に一度だけ判定済みのstatic boolean) を
        // チェックしてから何もしないか push/pop するだけなので、事前に
        // shouldAccelerate() で「今アクセラレートされているか」を判定してから
        // 呼び分ける必要はない。無条件に呼んでも安全な上、shouldAccelerate()
        // 自体の判定コスト（config参照を含む可能性がある）を毎フレーム
        // 支払わずに済む。
        ...
        try {
            ...
            // ==================== [FIX] ステンシル(レンズ穴)描画は一人称視点でのみ ====================
            // 経緯: プロファイル計測の結果、シェーダー(Iris)有効時にスコープ越しでFPSが大幅に
            // 低下する不具合の主因がここだった。TaczPolyMeshAttachmentModel(スコープ側)は
            // 元々 transformType.firstPerson() で一人称の時だけステンシル処理をするようゲート
            // されていたため、Irisのシャドウパス(影生成用の描画。一人称扱いにはならない)では
            // 正しくスキップされていた。しかしこちら(銃本体側)の同じステンシル処理には、この
            // ゲートが無かったため、シャドウパスでも毎回律儀に実行されてしまっていた。
            // 「レンズ穴からスコープの中身が見える」という視覚効果は一人称視点でしか意味を
            // 持たないため、それ以外(影生成・三人称等)では単純に通常描画へフォールバックする。
            final boolean isFirstPersonView = transformType.firstPerson();
            // ==================== [FIX2] シャドウパス中はpoly_mesh描画自体を丸ごとスキップ ====================
            // 経緯: [FIX]でステンシル部分はシャドウパス中スキップするようにしたが、プロファイル
            // 計測の結果、ステンシルを使わない通常のVBO描画(renderPolyMeshNormal等)自体も
            // シャドウパス中に毎回実行されており、これがまだ無視できないコストだった。
            // 影の形状はシャドウキャスト用にキューブボディ(super.renderで既に描画済み)だけで
            // 十分であり、poly_meshの細かい形状が影に反映されなくても実用上問題ない。
            // そのため、シャドウパス中はこのブロック全体を丸ごとスキップする。
            // (com.tacz.guns.compat.iris.IrisCompat.isRenderShadow() は内部でIris未導入なら
            // 即 false を返すので、Iris無し環境やシャドウパス以外への影響はない)
            ...
            } else if (hasScope && iAttachment != null && isFirstPersonView) {
            ...
            } else if (hasScope && iAttachment != null) {
                // 一人称以外(シャドウパス・三人称など): スコープ本体の見た目(位置・向き)は
                // 引き続き正しく描画しつつ、重いステンシル処理はスキップする。
                if (scopePosPath != null) {
                    poseStack.pushPose();
                    for (com.tacz.guns.client.model.bedrock.BedrockPart p : scopePosPath) {
                        p.translateAndRotateAndScale(poseStack);
    ...
    public void loadPolyMesh(ResourceLocation modelLocation) {
        try {
            ...
            try (var reader = new InputStreamReader(resource.open())) {
                ...
                com.example.taczmeshloader.render.ShaderStateTracker.registerModel(this.polyMeshModel);
