 ...
 * Iris がシェーダーパックを ON/OFF すると、シャドウパスの有無や
...
public class PolyMesh {
    ...
    // drawVBO() で毎フレーム使い回すスクラッチ行列。
    // 1.21.x の VertexBuffer#drawWithShader() は「現在のGLモデルビュー行列と
    // 合成済み」の行列を要求するため乗算自体は避けられないが、呼び出しの
    // たびに new Matrix4f(...) でオブジェクトを新規確保する必要はない。
    // 描画はレンダースレッド上でのみ行われ、値は都度使い切りのため
    // フィールドとして使い回して問題ない。
    ...
    public void drawVBO(Matrix4f posePose, int packedLight) {
        ...
        scratchModelView.set(RenderSystem.getModelViewMatrix()).mul(posePose);

        vbo.bind();
        vbo.drawWithShader(scratchModelView, RenderSystem.getProjectionMatrix(), RenderSystem.getShader());
        VertexBuffer.unbind();
    }
